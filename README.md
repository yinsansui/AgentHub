...
control-plane
  ...
  -> sessions + session_runs lifecycle state
```

The durable control-plane store uses PostgreSQL when `AGENTHUB_DATABASE_URL` is configured. Runtime events are appended to `session_events` with a global `BIGSERIAL` cursor, message metadata is projected into `messages`, completed item content is expanded into `message_blocks`, and `sessions` / `session_runs` track the active execution lifecycle. Streaming `item.delta` events stay in the event log and are not treated as final blocks. If no database URL is configured, the server falls back to an in-memory development store only.

Session reads follow a snapshot + replay + live pattern:

```text
POST /workspaces/{workspaceId}/sessions   # create a session, optionally with firstTurn
...
`sessionId` and `runId` are generated by the control-plane and returned in the create/turn responses.
