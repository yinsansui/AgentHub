...
Control-plane user-facing APIs require a login cookie. The current prototype ships one built-in account, `admin` / `admin`; smoke scripts log in automatically and reuse a cookie jar. For manual API calls, first call `POST /auth/login` and send the returned `agenthub_session` cookie with subsequent requests.

Workspaces are isolated by owner. `GET /workspaces` returns `{ id, name, ownerUserId }`; `id` is a system-generated UUID, `name` is the visible and editable label, and the first list call creates `Default 工作空间` for the current owner when none exists. URLs and API paths still use `workspaceId`, but that value is the generated UUID, not a user-editable id.
...
Then open the user portal and log in with `admin` / `admin`.
