      ...
      {createModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm" onClick={() => setCreateModalOpen(false)}>
          <div className="bg-apple-panel rounded-2xl shadow-apple-card p-6 w-full max-w-sm mx-4" onClick={(e) => e.stopPropagation()}>
            <h3 className="text-lg font-medium mb-4">新建 Workspace</h3>
            <form onSubmit={handleCreateSubmit}>
              <input
                ...
                className="w-full text-[13px] px-3 py-2 rounded-lg bg-apple-bg shadow-none border border-black/[0.08] mb-2"
              />
              {formError && <p className="text-apple-destructive-text text-[12px] mb-3">{formError}</p>}
              <div className="flex items-center justify-end gap-2">
                <button
                  type="button"
                  className="min-h-8 px-3 py-0 text-[13px] bg-transparent shadow-none text-apple-fg-50 hover:bg-apple-fg-5 rounded-lg"
                  onClick={() => {
                    setCreateModalOpen(false);
                    setFormName("");
                    setFormError(null);
                ...
                <button
                  ...
                  className="min-h-8 px-3 py-0 text-[13px] bg-apple-accent text-white shadow-none hover:bg-apple-accent-hover rounded-lg"
                >
                  {formLoading ? "创建中…" : "创建"}
                </button>
              </div>
