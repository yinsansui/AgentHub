...
func (c *AgentPodClient) PrepareSession(ctx context.Context, workspaceID, token string, prepare protocol.PrepareSessionRequest) error {
	payload, err := json.Marshal(prepare)
	if err != nil {
		return err
	}
	url := c.Endpoint(workspaceID) + "/sessions/" + prepare.SessionID + "/prepare"
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return fmt.Errorf("agent-pod prepare session returned %s: %s", resp.Status, strings.TrimSpace(string(body)))
	}
	return nil
}
